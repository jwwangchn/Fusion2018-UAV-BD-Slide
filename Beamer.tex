...
\maketitle
\note{Good afternoon, everyone. I am Jinwang Wang from Wuhan University. In the next few minutes, I will introduce our work ``Bottle Detection in the Wild Using Low-Altitude Unmanned Aerial Vehicles''.}
...
\begin{frame}[c]{Motivation}
	...
	\note{As we all know, It's very \cBlue{dangerous} for the sanitation workers who pick up bottles on the mountain. So can we help them? Yes! We can use UAV to pick up bottles.}
\end{frame}
...
%-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
\begin{frame}{Dataset}
	\justifying{To build a baseline for bottle detection in UAV images, we first establish a large scale dataset which called \cBlue{UAV-Bottle Dataset(UAV-BD).}}

	\note{To build a baseline for bottle detection in UAV images, we establish a large scale bottle detection dataset, we call \cBlue{UAV-Bottle Dataset(UAV-BD)}.}
\end{frame}
...
\begin{frame}{Dataset Collection}
	...
	\end{figure}
	\note{Eight scenes are chosen and annotated in our UAV-BD, including Bush forest land, Waste land, Step, Forest land, Flat land, Plastic stadium, Sand land, Grassland.}
\end{frame}
...
\begin{frame}{Dataset Statistics}
	\justifying{\footnotesize{The size of each original image(OI) in UAV-BD is \cBlue{$5472\times 3078$ pixels}, it's too large to be trained for CNN-based algorithms. Therefore we segment each OI into $144$ small subimages(SIs), and the size of each SI is \cBlue{$342\times 342$ pixels}. The numbers of images and instances in UAV-BD are shown in the table below.}}
	\begin{table}
		\centering
		\footnotesize
